...
static void ft_putull_hex(t_formatoptions *fo, unsigned long long num) {
  if (num >= 16) {
    ...
    if (num <= 9)
      count_bytes(&fo->byteswrotes, ft_putchar_fd(num + '0', fo->fd));
    else
      count_bytes(&fo->byteswrotes, ft_putchar_fd(num - 10 + 'a', fo->fd));
  }
}
...
static void ft_print_ullhash(t_formatoptions *fo, unsigned long long n,
                             char conversion) {
  if (fo->hash && n != 0) {
    fo->width -= 2;
    if (fo->zero) {
